...
const Fourweek = () => {
  ...
  useEffect(() => {
    const fetchWeekRanges = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/week-rangess');
  ...
  const handleWeekSelection = async (e) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/api/mbascoress/${startWeek}/${endWeek}`);
