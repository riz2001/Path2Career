...
const Mcaupdation = () => {
  const handleUpdateCourseYear = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/mcaupdateCourseYear'); // Adjust the API endpoint as needed
  ...
  const handleDeleteStudents = async () => {
    ...
    try {
      const response = await axios.delete('http://localhost:5000/api/mcadeleteStudents'); // Adjust the API endpoint as needed
      if (response.status === 200) {
